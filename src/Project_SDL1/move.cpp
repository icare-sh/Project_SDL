...
properties * render_copy_maj_pos(SDL_Renderer* renderer, SDL_Texture* texture, properties *prop, int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        if (prop[i].x + prop[i].speed * prop[i].direction_x / temp_taux < 0 || prop[i].x + prop[i].speed * prop[i].direction_x / temp_taux > 800 - SHAPE_SIZE)
        {
            prop[i].direction_x = -prop[i].direction_x;
        }
        if (prop[i].y + prop[i].speed * prop[i].direction_y / temp_taux < 0 || prop[i].y + prop[i].speed * prop[i].direction_y / temp_taux > 600 - SHAPE_SIZE)
        {
